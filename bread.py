...
cylinder_slice = Cylinder(vec([0.4,-.1,-1]),0.5,0.5,vec([.1,0,0]),brown)
bread_base_slice = Parallelepiped(vec([0.4,-0.4,-1.5]),vec([.1,0,0]),vec([0,0.3,0]),vec([0,0,1]),brown)
cut_cyl_slice = Parallelepiped(vec([0.4,-1.1,-1.5]),vec([2,0,0]),vec([0,1,0]),vec([0,0,2]),brown)

half_cylinder_slice = CSG()
half_cylinder_slice.add_children(CSG(cylinder_slice),CSG(cut_cyl_slice),"s")
bread_slice = CSG()
bread_slice.add_children(half_cylinder_slice,CSG(bread_base_slice),"u")
